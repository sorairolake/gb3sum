...
func TestParseUntaggedChecksum(t *testing.T) {
	...
	if checksum.unescapedFilename != "foo.txt" {
		t.Errorf(
			"expected unescaped filename `%v`, got `%v`",
			"foo.txt",
			checksum.unescapedFilename,
		)
	}
...
func TestParseUntaggedChecksumEscaped(t *testing.T) {
	...
	expectedDigest, err := hex.DecodeString(
		"bee561c8ab07fec8ec114df9ffff1fce5e1e31846483e110fe6f05582bd816be",
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if checksum.escapedFilename != "CODE\\\\_OF\\n_CONDUCT\\r.md" {
		t.Errorf(
			"expected escaped filename `%v`, got `%v`",
			"CODE\\\\_OF\\n_CONDUCT\\r.md",
